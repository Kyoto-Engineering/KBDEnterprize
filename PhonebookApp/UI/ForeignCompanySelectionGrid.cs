...
{
    public partial class ForeignCompanySelectionGrid : Form
    {
        ...
        public ForeignCompanySelectionGrid()
        {
            InitializeComponent();
        }
        ...
        private void ForeignCompanySelectionGrid_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                cmd = new SqlCommand();
                cmd.Connection = con;
                cmd.CommandText = "select Company.CompanyName,isnull(nullif(CompanyForeignAddress.FApartmentC,\'\') + \', \',\'\')+isnull(nullif(CompanyForeignAddress.FStreetC,\'\') + \', \',\'\') + isnull(nullif(CompanyForeignAddress.FStateC,\'\') + \', \',\'\') + isnull(nullif(CompanyForeignAddress.FCityC,\'\') + \', \',\'\') + isnull(nullif(CompanyForeignAddress.FZipcode,\'\') + \', \',\'\') + isnull(nullif(CompanyForeignAddress.FNearestlandmark,\'\') + \', \',\'\')+ isnull(nullif(Countries.CountryName,\'\') + \', \',\'\') as Addresss, Company.CompanyId FROM Company INNER JOIN CompanyForeignAddress ON Company.CompanyId = CompanyForeignAddress.CompanyId INNER JOIN Countries ON CompanyForeignAddress.CountryId = Countries.CountryId order by Company.CompanyId asc";
        ...
        private void textBox1_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                cmd = new SqlCommand();
                cmd.Connection = con;
                cmd.CommandText = "select Company.CompanyName,isnull(nullif(CompanyForeignAddress.FApartmentC,\'\') + \', \',\'\')+isnull(nullif(CompanyForeignAddress.FStreetC,\'\') + \', \',\'\') + isnull(nullif(CompanyForeignAddress.FStateC,\'\') + \', \',\'\') + isnull(nullif(CompanyForeignAddress.FCityC,\'\') + \', \',\'\') + isnull(nullif(CompanyForeignAddress.FZipcode,\'\') + \', \',\'\') + isnull(nullif(CompanyForeignAddress.FNearestlandmark,\'\') + \', \',\'\')+ isnull(nullif(Countries.CountryName,\'\') + \', \',\'\') as Addresss, Company.CompanyId FROM Company INNER JOIN CompanyForeignAddress ON Company.CompanyId = CompanyForeignAddress.CompanyId INNER JOIN Countries ON CompanyForeignAddress.CountryId = Countries.CountryId where Company.CompanyName like '" + textBox1.Text + "%' order by Company.CompanyId asc";
                rdr = cmd.ExecuteReader(CommandBehavior.CloseConnection);
                dataGridView1.Rows.Clear();
                while (rdr.Read() == true)
                {
